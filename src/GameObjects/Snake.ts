...
export class Snake {
    ...
    private sections: number = 20;
    private sectionLength: number = 20;
    ...
    public moveDirection(move: ControlVector): void {
        ...
        for (let index: number = 1; index < this.points.length; index++) {
            ...
            const distance: number = logic.distanceBetweenPoints(
                logic.pointABS(point), logic.pointABS(LastPoint)
            );
            ...
            if (distance >= this.sectionLength) {
                ...
                if (vector.x > 0)
                    point.x += moveDistance.x / speedForOtherSerctions;
                else
                    point.x -= moveDistance.x / speedForOtherSerctions;

                if (vector.y > 0)
                    point.y += moveDistance.y / speedForOtherSerctions;
                else
                    point.y -= moveDistance.y / speedForOtherSerctions;
            }
        ...
        // console.log(`
        //     =======
        //     speedForOtherSerctions : ${speedForOtherSerctions}, 
        //     =======
        // `)
    }
